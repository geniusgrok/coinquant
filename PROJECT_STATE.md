...
snapshot子单路径已审查；修正single-algo DELETE不应传symbol，30项受影响检查通过。
下一具体工程：补历史经济规则/动态FX与USDT估值来源可用性；
...
## 后续价差研究已推进
现货/永续相对价格机制已做成本量级筛选：48个月新spot档案校验完成，31缺失小时/8异常时间K线保留并停用。
35025有效小时中37小时价差超0.37%成本量级，稀疏仅2个独立机会，样本不足，不扩大策略实现。
见basis-screen-summary.json、basis-originals.json。未查看2024+现货，不重复围绕两个事件搜索。
补充原件完整保存，首批代码af7095c、撤单schema修正8498c98均已远端字节核验与CI通过；最新HEAD请重新核验。
当前没有后台研究任务。继续时优先落实历史估值/规则来源审计，记录缺口后再选择经济信息不同的新假设。
