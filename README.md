...
当前 `run` 完成同样的账户观察，并读取最近 120 根完整原生四小时行情后报告 `blocked`，原因是没有已验收模型与交易生命周期。
...
Binance 原生行情覆盖核验已通过：完整窗口 trade/mark 各 58,896 小时、funding 7,362 次。
月度 mark 原件共缺 216 小时，已通过 6 段官方 API 原生补页补齐，保留原件而不覆盖。
完整历史规则、资金费精确估值与 USDT 风险仍未闭合，数据完整不代表经济验收通过。
