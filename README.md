...
**尚未交付的能力：**离线保留入场挂单所需的母单余量与止损联动；普通挂单修改和保证金调整的完整持久化执行流程。适配器中存在对应接口方法，不代表端到端功能已经完成。当前入场使用有界 IOC 限价单，不留下未确认安全的入场余单；不能据此声称全部挂单需求已实现。
...
程序正常退出和释放本地锁不撤销有效保护。退出前取消并确认残余入场单；不把普通单查询失败当作没有订单。保护无法确认时禁止增加风险，必要时尝试已授权的只减仓。网络断开或结果未知时不能保证成功平仓，报告会保留未知状态和恢复意图，不会把超时当作失败重发。
