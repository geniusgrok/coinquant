...
def extend_minutes(prepared, roots, hours, warmup_identity=(), *, skip_invalid=False):
    ...
    for t in sorted(set(hours)):
        ...
        try:
            for kind in tables:
                ...
                selected.append((kind, rows, dict(validate_hour(rows,cache[0][kind][t],trade=kind=='klines'),kind=kind)))
        except ValueError as exc:
            if not skip_invalid:
                raise
            invalid.append(dict(time=t,reason=str(exc)))
            continue
        for kind, rows, check in selected:
            checks.append(check)
            for r in rows:
    ...
    combined=dict(hours=sorted(set(old_validation['hours'])|set(valid_hours)),
        original=old_validation,incremental_checks=checks,
        incremental_hours=sorted(set(valid_hours)-{t for t in old_tables['klines'] if t%HOUR==0}),
