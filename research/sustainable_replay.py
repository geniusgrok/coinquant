...
def run_account(root, output, prepared, label, *, full=False, stress=False,
    ...
    modes={'S60':'instant','SC60':'prepared','SX60':'sliced','PIR1':'sliced','DC10':'sliced'}
    cache,minutes,quotes,validation,data_id=prepared
    if (stop_risk_share is None) != (research_protocol is None) or (stop_risk_share is not None and label not in ('SX60','DC10')):
        raise ValueError('frozen stop-risk research requires its paired label and protocol')
    cfg=ExecutionStudy(True,stress,frozenset(validation['hours']),quotes,D(6),True,modes[label],stop_risk_share)
    reference='post_impulse_restart' if label=='PIR1' else 'drawdown_reclaim' if label=='DC10' else 'impulse_hold'
