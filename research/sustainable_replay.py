...
def run_account(root, output, prepared, label, *, full=False, stress=False,
    ...
    modes={'S60':'instant','SC60':'prepared','SX60':'sliced','PIR1':'sliced','T20':'sliced'}
    cache,minutes,quotes,validation,data_id=prepared
    if (stop_risk_share is None) != (research_protocol is None) or (stop_risk_share is not None and label not in ('SX60','T20')):
    ...
    reference='post_impulse_restart' if label=='PIR1' else 'daily_trend' if label=='T20' else 'impulse_hold'
