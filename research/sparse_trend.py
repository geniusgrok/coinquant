"""L1 research-only signal; never imported by the production trading path.
...
def signal(bars: list[Bar], now: int) -> Signal:
    ...
    if len(bars) < LOOKBACK + 1:
        raise Blocked('L1 needs 121 complete 4h candles')
    ...
    # Evidence of directional persistence beyond one realized RMS unit.
    # One continuous score, no regime voting or independent filter stack.
    conviction = max(ZERO, 1 - 1 / abs(score)) if score else ZERO
