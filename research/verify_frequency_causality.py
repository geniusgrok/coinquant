    ...
    for schedule in ('sparse',research_schedule):
        for modified in (False,True):
            ...
            with patch.object(replay,'inputs',changed if modified else original),contextlib.redirect_stdout(io.StringIO()):
                replay.run(root,Path('evidence/binance-boundary-20260921'),Path('evidence/binance-mark-repair-20260921'),out,root,False,schedule,Path('evidence/binance-boundary-20260921/current-instrument.json'),'one_campaign',allocation,reference,minute_days=minute_days,risk_scale=risk_scale)
