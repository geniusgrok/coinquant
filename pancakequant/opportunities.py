...
class Opportunities:
    ...
    def update(self, end, high, low, close):
        ...
        if self.mechanism=='squeeze':
        ...
        elif self.mechanism in ('shock','impulse','impulse_hold','persistent_impulse'):
            ...
            if not self.active and shock:
                ...
                if self.mechanism in ('impulse','impulse_hold','persistent_impulse'):
                    side=-side;stop=(prior+close)/2
                    take=close*(close/stop)**20 if self.mechanism in ('impulse_hold','persistent_impulse') else close+2*(close-stop)
                    if take>0:self.active=Opportunity(end,side,stop,take,None if self.mechanism=='persistent_impulse' else end+42*FOUR_HOURS)
                elif stop!=close:
