...
def run(root,output):
    ...
    for t,u in zip(schedule,schedule[1:]):
        ...
        for name,side in [('crowding',d),('long',1),('channel',channel)]:
            cash=ZERO
            for j in range(bisect_right(times,t),bisect_right(times,u)):
                r=rates[j];bar=marks[times[j]//HOUR*HOUR];price=D(bar[1] if times[j]%HOUR==0 else bar[2] if side*r>0 else bar[3])
