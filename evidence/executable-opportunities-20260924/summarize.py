...
def main(root):
    ...
    for name,predicate in (
        ...
        sections[name]=dict(total_events=len(eligible),known=stats([r for r in eligible if r['status']=='closed']),
                            unknown=len([r for r in eligible if r['status']!='closed']))
    ...
    summary=dict(protocol_sha256=json.loads((root/'FREEZE.json').read_text())['protocol_sha256'],
        all_events=len(rows),closed=stats(known),
