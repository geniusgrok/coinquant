...
class ChannelCore:
    ...
    def update(self, end, high, low, close):
        ...
        if self.active_since is not None and len(prior) >= 10 and close < min(b[1] for b in prior[-10:]):
            self.active_since = None
        elif self.active_since is None and len(prior) >= 20 and close > max(b[0] for b in prior[-20:]):
            self.active_since = end
        self.bars.append((high, low, close))
        self.last = end
        return (ChannelOpportunity(self.active_since, min(b[1] for b in list(self.bars)[-10:]))
